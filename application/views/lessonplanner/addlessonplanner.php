...
if(isset($_POST['submit'])) 
{
	...
	//die(); 
		...
		//echo "<br>";
			...
			$test = "select sem, week from ilearn_week where sem='$sem' and week='$week'";
			list($semm, $weekk)=mysql_fetch_row(mysql_query($test));
	
					if($semm!='$sem' && $weekk!='$week'){
						
						echo "sem tak same dengan week!!!";
					}
	...
	//--------------------------------------- ilearn_status --------------------------------------------------------------
			 ...
			 $stat = "select teacher_id from ilearn_status where teacher_id='$upload_id' AND week='$week' AND sem='$sem' AND year='$_POST[tahun]'";
		 list($tid)=mysql_fetch_row(mysql_query($stat));
	 
		 if($tid==''){
			 $sql ="INSERT INTO ilearn_status set teacher_id ='$upload_id', approve='', week='$week', sem='$sem', year='$_POST[tahun]', submit='1', waiting='1'";	 
			 mysql_query($sql);}
		 	else {
				/*?>$q = "select submit, waiting from ilearn_status where teacher_id='$upload_id' AND week='$week' AND sem='$sem' AND year='$_POST[tahun]'";
				list($sub,$wait)=mysql_fetch_row(mysql_query($q));

				$fsub = $sub+1;
				$fwait = $wait+1;
				
			 $sql ="update ilearn_status set submit='$fsub', waiting='$fwait' where teacher_id ='$upload_id' AND week='$week' AND sem='$sem' AND year='$_POST[tahun]'";	 
			 mysql_query($sql);<?php */
			 
			 	$rqstat="SELECT COUNT(*) FROM ilearn_content WHERE upload_id='$upload_id' AND sem='$sem' AND week='$week' AND year='$_POST[tahun]' AND flag='1' AND (approve_status='1' or approve_status='3')";
				list($tot_app)=mysql_fetch_row(mysql_query($rqstat));
					
				$sql ="update ilearn_status set submit='$tot_app', waiting='$tot_app' where teacher_id ='$upload_id' AND week='$week' AND sem='$sem' AND year='$_POST[tahun]'";	 
				mysql_query($sql);
...

{
	...
	<tr>
		...
		<td colspan="3"><textarea name="desc1" cols="80" rows="4" wrap="hard" ></textarea></td>
		<td>Notes:<br>
