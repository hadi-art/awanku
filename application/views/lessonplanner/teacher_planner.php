...
$sta="select teacher_id,submit,waiting, approve from ilearn_status where teacher_id='$uuid' AND week='$week' AND sem='$sem' AND year='$year'";
	...
	echo $url_page = $base."index.php/lessonplanner_control/teacher_planner/$sem/$week/$subject/$year";
	
	echo redirect($url_page);
...
{ 
	...
	$result=mysql_query($query);
		...
		// ----------------------------------------- approve dr table ilearn_status ----------------------------------
	...
	//$fwait=$waiting-1;
	if($approve==''){
			$fstat="UPDATE ilearn_status set approve='1' where teacher_id='$uuid' AND week='$week' AND sem='$sem' AND year='$year'";
			mysql_query($fstat);

	}else {
		$fapp=$approve+1;
	$fstat="update ilearn_status set approve='$fapp' where teacher_id='$uuid' AND week='$week' AND sem='$sem' AND year='$year'";
	mysql_query($fstat); }
...
{ 
	...
	$query="update ilearn_content set approve_status='1', approve_by='$sesi_user', approve_date='$date' where subject_id='$subject' AND sem='$sem' AND week='$week' AND year='$year' AND approve_status='3' AND flag='1'";
	$result=mysql_query($query);
	...
	$rqstat="SELECT upload_id,COUNT(*) as cv FROM ilearn_content WHERE subject_id='$subject' AND sem='$sem' AND week='$week' AND year='$year' AND flag='1'  AND approve_status='1' GROUP BY upload_id";
	$name=mysql_query($rqstat);
		
	while ($row = mysql_fetch_array($name)) {
 	$uuid=$row["upload_id"];
	$cv=$row["cv"];
	//echo $uuid;
		// echo "<br>".$stat = "select approve,teacher_id from ilearn_status where teacher_id='$uuid' AND week='$week' AND sem='$sem' AND year='$year'";
		 //list($app,$tid)=mysql_fetch_row(mysql_query($stat));
	 
			"<br>". $sql ="UPDATE ilearn_status set approve='$cv' where teacher_id ='$uuid' AND week='$week' AND sem='$sem' AND year='$year'";	 
			mysql_query($sql);
...
if(isset($_GET['duplicate']))
{
	...
	//echo redirect($base('/index.php/lessonplanner_control/teacher_planner/$sem/$week/$subject/$year'));
...
return $path;
	...
	//show lesson planner-------------------------------------------------------------------------------------------------------------
	if ($sesi_user=='699'|| $sesi_user=='22'|| $sesi_user=='25'|| $sesi_user =='26'|| $sesi_user =='23'|| $sesi_user =='39'){
	//show lesson planner-------------------------------------------------------------------------------------------------------------
	
	$query="Select * from ilearn_content where sem='$sem' AND week='$week' AND subject_id= '$subject' AND year='$year' AND flag= '1' AND (approve_status='1' or approve_status='3') ORDER BY date, ftime";
	$result=mysql_query($query);
...
<?php /*?><a href="<?php echo $site."/lessonplanner_control/addlessonplanner/0/$sem/$week/$subject/add" ?>" title="Add :<?php echo view_subject($subject); ?>" onclick="window.open(this.href, 'child', 'scrollbars,width=650,height=600'); return false" ><img src ="<?php echo $base; ?>/images/document_add.png" width = "25" height = "25"/>Add</a>
<?php */?>
<br />
<div id="tablektube" style="width:auto">
<a href="<?php echo $site."/lessonplanner_control/my_planner/$sem/$week/$subject/$year?id=$sesi_user" ?>">My LessonPlanner</a>
<br /><br />
...
<tr>
	...
	<th><center>Date </center></th>
	<th><center>Time </center></th>
<?php /*?>	<th><center>T.Name </center></th>
<?php */?>    <th><center>Class </center></th>
	<th><center>Learning Area(s)</center></th>
<?php /*?>	<th><center>Learning Activities</center></th><?php */?>	
	<th><center>Files</center></th>
   	<th><center>Review</center></th>
...
<tr> 	  	
			<td><?php echo $bil;?></td>
      		<td><?php echo $hariapadong."<br>".date('d-m-Y', strtotime($datee)); ?></td>
			<td><?php print $row['Ftime']?> - <?php print $row['Ttime']?></td>
<?php /*?>			<td><?php getfnama($row['upload_id']); ?></td>
<?php */?>	
